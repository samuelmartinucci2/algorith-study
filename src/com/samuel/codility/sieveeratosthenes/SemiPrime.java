...
public class SemiPrime {
    ...
    static class Solution {
        public int[] solution(int N, int[] P, int[] Q) {
            ...
            List<Integer> primeNumbers = new ArrayList<>(nonPrimes.length);
            for (int i = 2; i <= N; i++) {
                if (nonPrimes[i]) continue;
                primeNumbers.add(i);
            }
            ...
            for (int prime1 : primeNumbers) {
                for (int prime2 : primeNumbers) {
                    if (prime1 * prime2 > N) break;
                    semiPrimes[prime1 * prime2] = 1;
                }
            }

            for (int i = 4; i < semiPrimes.length; i++) {
            ...
            for (int i = 0; i < P.length; i++) {
                ...
                rtrn[i] = semiPrimes[end] - semiPrimes[start - 1];
            }
